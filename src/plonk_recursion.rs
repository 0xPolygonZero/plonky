...
use crate::{
    AffinePointTarget, Circuit, CircuitBuilder, CurveMulOp, Field, HaloEndomorphismCurve,
    PublicInput, Target, NUM_CONSTANTS, NUM_ROUTED_WIRES, NUM_WIRES,
...
pub fn recursive_verification_circuit<C: HaloEndomorphismCurve>(
    ...
    let zeta =
        builder.rescue_hash_n_to_1(&[vec![alpha], flatten_points(&proof.c_plonk_t)].concat());
    let (v, u, x) = builder.rescue_hash_n_to_3(&[vec![zeta], proof.all_opening_targets()].concat());
...
fn flatten_points(points: &[AffinePointTarget]) -> Vec<Target> {
    let coordinate_pairs: Vec<Vec<Target>> = points.iter().map(|p| p.to_vec()).collect();
...
fn verify_ipa<C: HaloEndomorphismCurve>(
    ...
    // Compute [c] [n(x)] u = [c] u'.
    let u_n_x_c = builder.curve_mul::<C>(CurveMulOp {
        scalar: c,
        point: u_prime,
    });
