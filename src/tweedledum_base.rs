...
impl TweedledumBase {
    ...
    const ORDER: [u64; 4] = [11619397960441266177, 255193519591741881, 0, 4611686018427387904];

    /// Twice the order of the field: 57896044618658097711785492504343953926644407311910638113546634138727284211714
    const ORDER_X2: [u64; 4] = [4792051847172980738, 510387039183483763, 0, 9223372036854775808];

    /// R in the context of the Montgomery reduction, i.e. 2^256 % |F|.
    const R: [u64; 4] = [2035294266095304701, 17681163514934325971, 18446744073709551615, 4611686018427387903];

    /// R^2 in the context of the Montgomery reduction, i.e. 2^(256*2) % |F|.
    const R2: [u64; 4] = [2885853259929485328, 10494584067553537908, 15959394653775906393, 56485833754855950];

    /// R^3 in the context of the Montgomery reduction, i.e. 2^(256*3) % |F|.
    const R3: [u64; 4] = [11023471670160566071, 18013763770685241468, 7203328081223416457, 2412999303287602290];
...
impl Field for TweedledumBase {
    ...
    const ONE: Self = Self { limbs: [2035294266095304701, 17681163514934325971, 18446744073709551615, 4611686018427387903] };
    const TWO: Self = Self { limbs: [10897934645458894841, 16660389436567358444, 18446744073709551615, 4611686018427387903] };
    const THREE: Self = Self { limbs: [1313830951112933365, 15639615358200390918, 18446744073709551615, 4611686018427387903] };
    ...
    const NEG_ONE: Self = Self { limbs: [9584103694345961476, 1020774078366967526, 0, 0] };
