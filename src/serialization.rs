use crate::{Bls12377Base, Bls12377Scalar, Curve, Field, TweedledeeBase, TweedledumBase, AffinePoint, Tweedledee, Tweedledum, Bls12377};
    ...
    ($curve:ty, $field:ty) => {
        ...
        impl ToBytes for AffinePoint<$curve> {
            fn write<W: Write>(&self, mut writer: W) -> Result<()> {
                ...
                let odd = if self.y.to_canonical_u64_vec()[0] % 2 == 1 {2} else {0};
        ...
        impl FromBytes for AffinePoint<$curve> {
            fn read<R: Read>(mut reader: R) -> Result<Self> {
                ...
                let mask = mask[0];
                if mask & 1 == 1 {
                    return Ok(AffinePoint {x: <$field>::ZERO, y: <$field>::ZERO, zero: true});
                }
                ...
                let y = square_candidate.square_root().ok_or(Error::new(ErrorKind::Other, "Invalid x coordinate"))?;
...
macro_rules! test_curve_serialization {
    ($curve:ty, $basefield:ty, $test_name:ident) => {
        ...
        fn $test_name() -> Result<()> {
            ...
            p.write(&mut buf)?;
            let mut arr = [0u8; <$basefield>::BYTES + 1];
            arr.copy_from_slice(buf.as_slice());
            let q = AffinePoint::<$curve>::read(&arr[..])?;
            assert_eq!(p, q);
            Ok(())
        }
    };
...
mod test {
    ...
    test_curve_serialization!(Tweedledee, <Tweedledee as Curve>::BaseField, test_tweedledee_curve_serialization);
    test_curve_serialization!(Tweedledum, <Tweedledum as Curve>::BaseField, test_tweedledum_curve_serialization);
    test_curve_serialization!(Bls12377, <Bls12377 as Curve>::BaseField, test_bls_curve_serialization);
}
